...
if __name__ == "__main__":
    ...
    for i in range(len(test_files)):
        ...
        print(float(mean_squared_error(t2_gen, t2_gt)))
        # print(ssim(t2_gen, t2_gt))
        mae_by_t1[Path(test_files[i].get('t1w')).with_suffix('').with_suffix('').name] = {}
        mae_by_t1[Path(test_files[i].get('t1w')).with_suffix('').with_suffix('').name]['t2gen_vs_t2gt'] = float(mean_squared_error(t2_gen, t2_gt))
        mae_by_t1[Path(test_files[i].get('t1w')).with_suffix('').with_suffix('').name]['t2gt_vs_t2gt'] = float(mean_squared_error(t2_gt, t2_gt))
        mae_by_t1[Path(test_files[i].get('t1w')).with_suffix('').with_suffix('').name]['t1gt_vs_t2gt'] = float(mean_squared_error(t1_gt, t2_gt))
         

# print(mae_by_t1)
with open('mean_squared_error.json', 'w') as outfile:
